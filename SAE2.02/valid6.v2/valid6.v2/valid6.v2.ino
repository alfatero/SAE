...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  static bool muted = false;        // conserve l’état muet entre les appels
  if (!muted) {
    volumeprec = volume;            // stocke le volume avant mute
    audio.setVolume(100);           // 100 = atténuation max → silence
    volume = 0;                     // met à jour la variable pour le statut MQTT
    Serial.println("🔇 Son muté");
  } 
  else {
    volume = volumeprec;            // restaure le volume précédent
    audio.setVolume(volume);
    Serial.print("🔊 Son restauré -> ");
    Serial.println(volume);
    }
  muted = !muted;                   // inverse l’état pour le prochain appui
  publishStatus();
  }
